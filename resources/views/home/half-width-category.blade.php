<a href="{{ route('products.index', $category->slug) }}" class="col-md-6 p-0 background-image d-flex flex-column {{ ($loop->index+2) %3 == 0 ? 'align-items-md-end' : 'align-items-start' }}" style="background-image: url('{{$category->getHomeImage()}}'); height: 55vh" title="{{ ucfirst($category->name) }}">
